...
namespace WPPluginSkeleton_Vendor\BrainMaestro\GitHooks;
...
class Hook
{
    ...
    {
        return \array_flip(['applypatch-msg', 'commit-msg', 'post-applypatch', 'post-checkout', 'post-commit', 'post-merge', 'post-receive', 'post-rewrite', 'post-update', 'pre-applypatch', 'pre-auto-gc', 'pre-commit', 'pre-push', 'pre-rebase', 'pre-receive', 'prepare-commit-msg', 'push-to-checkout', 'update']);
    }
    /**
     * Check if a hook is valid
     * @param string $dir
    ...
    private static function getComposerJson($dir)
    {
        ...
        return (array) \json_decode($contents, \true);
    }
}
