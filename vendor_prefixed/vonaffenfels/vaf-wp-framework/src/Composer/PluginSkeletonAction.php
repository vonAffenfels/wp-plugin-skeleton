...
class PluginSkeletonAction
{
    ...
    public static function onPostCreateProject(Event $event) : void
    {
        ...
        $replacementArray = ['%%PLUGIN_NAME%%' => $pluginName, '%%PLUGIN_DESCRIPTION%%' => $pluginDescription, '%%PLUGIN_SLUG%%' => $slug, '%%PLUGIN_NAMESPACE%%' => $namespace, '%%VENDOR_NAMESPACE%%' => $vendorNamespace, '%%WEBSITE%%' => $website, '%%AUTHOR_COMPLETE%%' => $authorComplete, '%%AUTHOR_NAME%%' => $authorName, '%%AUTHOR_EMAIL%%' => $authorEmail];
        $filesToPatch = ['config/services.yaml', 'src/Plugin.php', 'wp-plugin-skeleton.php', 'scoper.inc.php'];
        ...
        $composerData['scripts']['post-autoload-dump'] = ['WPPluginSkeleton_Vendor\\VAF\\WP\\Framework\\Composer\\PluginActions::prefixDependencies'];
        $authorData = [];
        if (!empty($authorName)) {
